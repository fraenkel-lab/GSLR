...
\begin{column}{.3\linewidth}
	...
	\begin{block}{Synthetic Data}
		...
		Since we don't have the ground truth subgraph for our real data, we create synthetic data by sampling from the multivariate gaussian defined by the real data, then translating "positive" examples by a "perturbation" vector.
		$negative = \mathcal{N} (\vec{\mu}, \bm{\Sigma} ) \quad positive = \mathcal{N} (\vec{\mu}, \bm{\Sigma} ) + \vec{x}$
		...
		3.3/(2*pi*\sa*\sb) * exp(-((x-\ma)^2/\sa^2 + (y-\mb)^2/\sb^2))/2;}]
		\begin{axis}[
		...
		\node at (axis cs:-1,1,0.18) [pin=195:$P(x_1)$] {};
		\node at (axis cs:0.5,4,0.32) [pin=140:$P(x_2)$] {};
		\node at (axis cs:2.6,4,0.26) [pin=-3:$P(x_2*)$] {};
		\end{axis}
		\end{tikzpicture}
