...
if __name__ == "__main__":
	...
	inputs = [(pathway_id, project_root+'generated_data/ludwig_svd_normals_guassian_mixture/'+pathway_id+'_inbiomap_exp.csv', nodes, edges, costs) for pathway_id in pathways_df.index.get_level_values(2)]

	pool = multiprocessing.Pool(n_cpus)
	results = pool.map(GSLR, inputs)

	pickle.dump(results, open('gslr_gmm_results.pickle', 'wb'))
